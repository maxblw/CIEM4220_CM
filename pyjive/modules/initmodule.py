        ...
        with open(fname) as msh:
            for line in msh:
                ...
                if 'node' in line or 'nodes' in line:
                    ...
                    sections.append(int(sp[3]) if len(sp) > 3 else 0)

        nN = len(nodes)
        inode = nN
        imember = 0

        for mem, nel, sec in zip(members, nelem, sections):
            ...
            if nel <= 0:
                ...
                elems.append(Element(connectivity, sec))
